...
1. Extract the ZIP file. Do not run the build from inside the ZIP.
2. Open the extracted project folder and confirm it contains `Dockerfile`,
   `app.py`, and `requirements.txt`.
3. Open PowerShell or another terminal in that folder. This folder is the
   project root.
...
The period (`.`) at the end is required. It tells Docker to use the current
folder as the build context. If Docker reports that it cannot find
`Dockerfile`, confirm that the terminal is open in the extracted project root
